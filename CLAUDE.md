...
| `homepage/` | Five presentational section components (`AnnouncementBarSection`, `HeroSection`, `CategoriesSection`, `BestsellersSection`, `FeaturesSection`) shared by the public `HomePage` and the admin live preview. (`AboutSection` was removed — about page copy now lives on the dedicated `/about` route, not the homepage.) |
...
| `AccountPage.jsx` | Profile header + menu rows. Every menu item links to a real sub-route — no more "Coming Soon" modals. Items: My Orders, Inbox, Wishlist, Payment Settings, Account & Security, Notification Preferences, About, Contact, Terms, Privacy, Sign Out. |
| `AccountEditPage.jsx` | Edit display name. Email read-only (changing email needs a separate verification flow). Calls `PATCH /auth/me`. |
| `AccountSecurityPage.jsx` | Change password (knowing current) via `POST /auth/change-password`, and **Close Account** (real `DELETE /auth/me` self-delete — backend blocks if the user has active orders). |
| `InboxPage.jsx` | Derived inbox: order status updates from the last 30 days (no new DB table). Read/unread state persisted in `localStorage` under `kb_inbox_read`. |
| `WishlistPage.jsx` | Grid of saved products via `GET /api/wishlist`. Remove via heart toggle or in-card button. |
| `PaymentSettingsPage.jsx` | **Informational** — explains "we don't store cards" (Paystack handles checkout) + shows payment methods used (derived from `orders.payment_method`). No card storage. |
| `NotificationPreferencesPage.jsx` | 3 toggles: order status emails, promotional emails, account security emails (read-only, always on). Persisted to `users.notify_order_updates` / `notify_promotions`. |
| `TermsPage.jsx`, `PrivacyPage.jsx` | Markdown-rendered legal pages. Content sourced from `brand.termsContent` / `brand.privacyContent` (admin-editable via Settings → Store Branding → Legal pages). Uses `src/components/SimpleMarkdown.jsx` — no markdown dependency. |
| `LoginPage.jsx` | Email + password login. Shows a suspension banner when arriving with `?suspended=1` (set by the axios interceptor when a mid-session 403 says "suspended"). |
...
| `AboutPage.jsx` | Brand story page. All copy (hero label/heading/subtitle, story heading + body, stats array, reasons array) comes from `useBrand()` and is admin-editable via Settings → Store Branding → About page. |
...
| `AdminCustomersPage.jsx` | Customer table with **Role** dropdown (Customer/Admin, with confirm modal), **Suspend/Unsuspend** toggle, **View** detail modal (profile + lifetime stats + last 10 orders), **Delete** account (fires `account_goodbye` email). Self-protection: can't demote, suspend, or delete yourself. |
...
├── /orders, /orders/:id           OrdersPage, OrderDetailPage
├── /account                       AccountPage
├── /account/edit                  AccountEditPage
├── /account/security              AccountSecurityPage
├── /account/inbox                 InboxPage
├── /account/wishlist              WishlistPage
├── /account/payment               PaymentSettingsPage
├── /account/notifications         NotificationPreferencesPage
...
- ✅ **RESOLVED — About system entirely hardcoded and not admin-editable** (2026-06-04) — fixed 5 broken connections that made the `/about` page un-editable end-to-end despite partial backend support: (1) **Backend `routes/settings.js`**: added 5 `about_*` keys (`about_hero_label`, `about_hero_heading`, `about_hero_subtitle`, `about_story_heading`, `about_story`) to `PUBLIC_KEYS` so the storefront can fetch them. (2) **Backend `config/migrate.js`**: seeded sensible defaults for all 5 `about_*` keys (matching `brand.js`) via `ON CONFLICT DO NOTHING`. (3) **Backend `config/schema.sql` + `config/migrateHomepage.js`**: removed the orphaned `about` row from `homepage_sections` seed + added a one-shot `DELETE FROM homepage_sections WHERE section_key = 'about'` cleanup for existing DBs. (4) **Frontend `src/pages/admin/AdminSettingsPage.jsx`**: added 5 fields to `BRANDING_KEYS`, `brandingForm` state, settings loader, and a new "About page" sub-section in the Store Branding form (long story is a 6-row `<textarea>`); removed the misleading help comment that told admins to edit `brand.js`. (5) **Frontend `src/components/Footer.jsx`**: removed the broken Privacy and Terms links that both pointed at `/about`. (6) **Frontend `src/components/homepage/AboutSection.jsx`**: deleted the orphaned component (was never imported anywhere). (7) **`src/config/brand.js`**: updated header comment to accurately describe what is/isn't admin-editable. (8) **Doc**: updated `kingdom-books-api/CLAUDE.md` to document the by-key route + drop `about` from seeded section keys list. **No backend restart strictly required** — `migrate.js` re-run is enough to seed the defaults, but storefront sees changes after a normal page reload. `AboutPage.jsx` JSX was not modified (already reads via `useBrand()`); stats and reasons stay hardcoded — Tier 3 work. **IMPORTANT — action needed**: run `npm run migrate` in `kingdom-books-api` so the `about_*` settings get seeded and the orphan `about` homepage row gets deleted.
- ✅ **RESOLVED — About Tier 3 + Account overhaul + Coming Soon pages built** (2026-06-04) — covers **Tier 3 About** (admin-editable stats + reasons) plus a full **Account system overhaul** that fixes critical security/honesty bugs and ships every previously-stubbed page. (1) **Tier 3 About**: `about_stats_json` + `about_reasons_json` added to `ALLOWED_SETTING_KEYS` and `PUBLIC_KEYS`; seeded in `migrate.js`. `AdminSettingsPage` Store Branding → About page gained repeater UIs for the 4 stats and 4 reasons (serialized to JSON on save). `BrandContext.SETTINGS_MAP` now parses JSON-array keys safely with `brand.js` fallback. (2) **Self-delete (A1)**: real `DELETE /auth/me` cascades user; rejects when there are active orders. `closeAccount()` exposed on `AuthContext`; wired to a confirmation modal in the new `/account/security` page. Fires existing `account_goodbye` template. (3) **Suspended-user middleware (A2)**: `middleware/auth.js` now `SELECT suspended FROM users WHERE id = $1` per request — suspended users get 403 mid-session, no waiting for the 7-day token. Frontend `api/client.js` interceptor catches that 403, clears the token, redirects to `/login?suspended=1`; `LoginPage` shows a friendly banner when arriving with that flag. (4) **Role-change UI (A3)**: dead `setCustomerRole` import is now wired — `AdminCustomersPage` has a Customer/Admin `<select>` per row with a confirmation modal; backend already enforced self-demote protection. (5) **Edit Profile (H1)**: `PATCH /auth/me` (name only — email change deferred to a future verification flow). `AuthContext.updateProfile()` exposed. New `/account/edit` page. (6) **Change Password (H2)**: `POST /auth/change-password` validates current password, bcrypt-hashes new, fires a new `password_changed` email template. New `/account/security` form. (7) **Inbox**: `/account/inbox` derives messages from the user's order history (last 30 days, status updates) — no new DB table. Read state in `localStorage`. (8) **Wishlist**: new `routes/wishlist.js` (`GET/POST/DELETE`) on the existing `wishlist` table; new `WishlistProvider` context shares state across `ProductPage`, `ShopPage`, and a new `/account/wishlist` page. Heart icons on cards + detail page; anonymous click bounces to `/login`. (9) **Payment Settings**: `/account/payment` is informational — explains "we don't store cards" + lists payment methods used from order history. No card storage. (10) **Notification Preferences**: 2 new boolean columns on `users` (`notify_order_updates`, `notify_promotions`) via `migrateUserPreferences.js`; `GET/PATCH /auth/me/preferences`; new `/account/notifications` page with 3 toggles (the 3rd, account security, is read-only — always sent). (11) **Terms + Privacy**: new `/terms` and `/privacy` routes; markdown content sourced from `brand.termsContent` / `brand.privacyContent` (admin-editable via Settings → Store Branding → Legal pages). New `terms_content` / `privacy_content` settings keys + `SimpleMarkdown.jsx` component (no new dependency). (12) **Admin customer detail modal (M2)**: new `GET /admin/customers/:id` + `GET /admin/customers/:id/orders`; clicking a customer name in `AdminCustomersPage` opens a modal with profile, lifetime totals, last 10 orders. (13) **AccountPage rewrite**: removed `ComingSoonModal` and `CloseAccountModal` (moved real close-account to `/account/security`); every menu row now navigates to a real page. (14) **Footer + Navbar**: Privacy/Terms links restored (point to new pages); desktop AccountMenu dead-end items routed to their new pages; dropped "More Settings" (no clear use case). (15) **Migrations chained**: `migrate.js` now runs first in `npm run migrate` (was previously unmapped); `migrateUserPreferences.js` appended. Railway picks both up automatically via existing `railway.json` startCommand. (16) **Docs**: backend CLAUDE.md updated with new `/auth/me*`, `/wishlist`, `/admin/customers/:id*` endpoints + `users` table new columns + seeded settings list. **IMPORTANT — action needed**: push triggers Railway redeploy → migrations auto-run (adds the 2 user-preference columns + seeds `about_stats_json`, `about_reasons_json`, `terms_content`, `privacy_content`, and `password_changed` email template). Vercel auto-deploys frontend. ⚠️ Known Issue list — this single task burned through every actionable unresolved item (real `Close Account`, suspended-user middleware, dead-code `setCustomerRole`, orphan `wishlist` table, broken footer Privacy/Terms, "Coming Soon" page placeholders, admin customer detail modal). What's left in the Known Issues list is all backend-blocked or scope-bigger (email verification, brute-force protection, JWT refresh, payment method storage, notifications/inbox real-time, audit log, no tests).
...
| About hero label | `brand.aboutHeroLabel` / `about_hero_label` | Settings → Store Branding → About page |
| About hero heading | `brand.aboutHeroHeading` / `about_hero_heading` | Settings → Store Branding → About page |
| About hero subtitle | `brand.aboutHeroSubtitle` / `about_hero_subtitle` | Settings → Store Branding → About page |
| About story heading | `brand.aboutStoryHeading` / `about_story_heading` | Settings → Store Branding → About page |
| About story body | `brand.aboutStory` / `about_story` | Settings → Store Branding → About page |
| About stats / reasons | `brand.stats` / `brand.reasons` (hardcoded) | Not yet admin-editable — edit `src/config/brand.js` |
